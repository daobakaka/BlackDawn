...
namespace BlackDawn.DOTS
{
    ...
    struct ApplyPropDamageJob : IJobFor
    {
        ...
        public void Execute(int i)
        {
            ...
            float finalDamage = (instTotal + dotTotal) * (1f - a.damageReduction) * (1 - rd)*(1+db.damageAmplification+db.scorchMarkdamageAmplification)*(1+elementShieldBAddDamagePar+advanceADamagePar);
            //这里分离dot伤害
            float finalDotDamage = (dotTotal) * (1f-a.damageReduction) * (1-rd) * (1 + db.damageAmplification+db.scorchMarkdamageAmplification)*(1+elementShieldBAddDamagePar+advanceADamagePar);
